...
int	ft_check_map(t_source *src, char **map, int i, int j)
{
	...
	if (src->cou_exit < 1 || src->cou_plyr != 1 || src->cou_c < 1
		|| src->max_y < 4 || src->max_x < 4 || src->cou_enmy != 1)
		return (1);
	return (0);
}
...
void	put_pik_p(t_source *src, int x, int y, char map)
{
	if (map == 'W')
	{
		mlx_put_image_to_window(src->mlx, src->win, src->enemy_0, \
	...
	{
		mlx_put_image_to_window(src->mlx, src->win, src->plyr_r, \
						x * src->size, y * src->size);
		src->plyr_x = x;
		src->plyr_y = y;
	}
		...
		while (src->map[y][++x] != '\n' && src->map[y][x] != '\0')
		{
			...
			if (src->map[y][x] == 'E')
				mlx_put_image_to_window(src->mlx, src->win, src->exit, \
					x * src->size, y * src->size);
			if (src->map[y][x] == 'P' || src->map[y][x] == 'W')
				put_pik_p(src, x, y, src->map[y][x]);
		}
	}
}
